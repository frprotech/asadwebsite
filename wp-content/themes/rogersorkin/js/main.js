...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 8. CONTACT FORM SUBMISSION HANDLER (Connected to WordPress admin-ajax.php)
  ...
  if (contactForm && formFeedback && submitBtn) {
    contactForm.addEventListener('submit', (e) => {
      ...
      if (typeof rsData !== 'undefined' && rsData.ajax_url) {
        const formData = new FormData(contactForm);
        formData.append('action', 'rs_contact_submit');
        formData.append('security', rsData.nonce);

        fetch(rsData.ajax_url, {
          method: 'POST',
          body: formData
        })
        .then(res => res.json())
        .then(data => {
          if (data.success) {
            ...
            formFeedback.innerText = data.data.message || 'Thank you! Your message has been sent successfully. Roger will respond within 24 hours.';
            contactForm.reset();
          } else {
            formFeedback.className = 'form-feedback error';
            formFeedback.innerText = data.data.message || 'An error occurred. Please try again.';
          }
